...
class automatic_map:
    ...
    def map_init(self):
        #地图初始化
        self.map = Map(width=6, height=12, title='ABU Map',grid=0.5)
        #实体初始化
        self.entity_list = [
        ...
        #R2初始化
        self.cat = Cat((1.4,11.6),radius=0.3,move_acceleration=3,friction=1,controllable=False)
        print(f"cat.x: {self.cat.x}, cat.y: {self.cat.y}")
    def my_cat_control(self):
        self.cat.vx = 1
    def add_entity_and_start(self):
        for i in self.entity_list:
            self.map.add_entity(i)
        self.map.add_entity(self.cat)
        self.my_cat_control()
        print(f"cat.x: {self.cat.x}")
        print(f"cat.y: {self.cat.y}")
        self.map.start_simulation(interval=0.02)

def main():
    auto_map = automatic_map()
    auto_map.add_entity_and_start()
